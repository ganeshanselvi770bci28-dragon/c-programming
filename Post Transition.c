...
void send_all_packages(town* source, int source_office_index, town* target, int target_office_index) {
    ...
    package* accepted_pkgs = NULL;
    if (accepted_count > 0) {
        accepted_pkgs = malloc(accepted_count * sizeof(package));
    }
    
    package* rejected_pkgs = NULL;
    if (rejected_count > 0) {
        rejected_pkgs = malloc(rejected_count * sizeof(package));
    }
